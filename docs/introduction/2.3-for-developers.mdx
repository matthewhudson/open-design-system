...
1. **Node.js**:
   ...
   - We use `nvm` (Node Version Manager) to manage Node versions. `nvm` allows you to switch between different versions of Node.js easily.
   - **Installation**:

     - Follow the installation guide for your operating system from the [nvm repository](https://github.com/nvm-sh/nvm#installing-and-updating).
     ...
     - This command switches to the Node.js version specified in the `.nvmrc` file in the project.

2. **Yarn**:

   - Yarn is a package manager that doubles down as a project manager.
   - **Installation**:
     - Follow the instructions on the [official Yarn website](https://classic.yarnpkg.com/en/docs/install) to install Yarn.
